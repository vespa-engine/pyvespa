...
| Name                   | Type                 | Description                                                                                                                                                                                                                               | Default     |
| ---------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `tenant`               | `str`                | Tenant name registered in the Vespa Cloud. Must start with a lowercase letter, may only contain lowercase letters, digits and dashes, may not contain double-dashes, may not end with a dash, and may contain no more than 40 characters. | *required*  |
| `application`          | `str`                | Application name in the Vespa Cloud. Same naming rules as for the tenant.                                                                                                                                                                 | *required*  |
...
| `instance`             | `str`                | Name of the application instance. Same naming rules as for the tenant, except that "default" is allowed, while names ending in "-t" are reserved for tester instances. Default is "default".                                              | `'default'` |
...
Production tests in /tests/production-test/ (declared with `ProductionTest` on the application package, or hand-written JSON/YAML files) are validated before submission, as `vespa prod deploy` does, so a typo fails here rather than in the deployment pipeline. They only run at `Test` steps in the deployment configuration. See https://docs.vespa.ai/en/reference/applications/testing-production.html.
