...
### `MatchPhaseRanking(attribute, order, max_hits=None, total_max_hits=None)`
...
| `max_hits`       | `int` | Maximum number of hits per content node to pass to the next phase.         | `None`     |
| `total_max_hits` | `int` | Maximum number of hits across all content nodes to pass to the next phase. | `None`     |
...
| Name   | Type  | Description                                              | Default    |
| ------ | ----- | -------------------------------------------------------- | ---------- |
| `name` | `str` | Region id, e.g. "aws-us-east-1c". See Vespa Cloud zones. | *required* |
...
A delay step in deployment.xml (`<delay>`), which must pass after all previous steps before subsequent steps may proceed. Typically placed between a `Region` and a `Test` to gather metrics before a production test evaluates them.

Parameters:

| Name      | Type  | Description                      | Default |
| --------- | ----- | -------------------------------- | ------- |
| `hours`   | `int` | Hours to delay. Defaults to 0.   | `0`     |
| `minutes` | `int` | Minutes to delay. Defaults to 0. | `0`     |
| `seconds` | `int` | Seconds to delay. Defaults to 0. | `0`     |
...
A production test step in deployment.xml (`<test>`). Runs the production tests in `tests/production-test/` against the given region, which must have been deployed to in an earlier `Region` step. If a test fails, the rollout stops and later steps do not run.

Parameters:

| Name     | Type  | Description                                                 | Default    |
| -------- | ----- | ----------------------------------------------------------- | ---------- |
| `region` | `str` | Id of the production region to test, e.g. "aws-us-east-1c". | *required* |
...
### `DeploymentConfiguration(environment, regions=None, steps=None)`

Bases: `object`

Create a DeploymentConfiguration, which defines how to generate a deployment.xml file (for use in production deployments).

Pass either `regions`, for a plain sequential rollout, or `steps`, to also add `Delay` and `Test` (production test) steps between regions. A plain string in `steps` is shorthand for `Region(name)`.
...
| `regions`     | `list[str]`    | List of regions to deploy to, e.g. ["us-east-1", "us-west-1"]. See Vespa documentation for more information. | `None`     |
| `steps`       | \`list\[Region | Delay                                                                                                        | Test       |
...
### `ProductionTest(metric, duration, name=None, min=None, max=None, validate_metric_preset=True)`

Bases: `object`

A production test: a metric preset checked against a min and/or max bound over a time window, evaluated by Vespa Cloud after a production region has been deployed. Tests are written to `tests/production-test/` in the application package and run at each `Test` step in the `DeploymentConfiguration`. A failing test stops the rollout.

Parameters:

| Name                     | Type    | Description                                                                                                                                                                                                                  | Default    |
| ------------------------ | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `metric`                 | `str`   | Name of a metric preset, e.g. "cpu-utilization-container" or "query-latency-p95".                                                                                                                                            | *required* |
| `duration`               | `str`   | Aggregation window, a positive whole number followed by s, m, h or d, e.g. "10m". Vespa Cloud first waits this long, then aggregates the metric over the same window.                                                        | *required* |
| `name`                   | `str`   | Display name used in logs and failure messages.                                                                                                                                                                              | `None`     |
| `min`                    | `float` | Inclusive lower bound. At least one of min and max is required.                                                                                                                                                              | `None`     |
| `max`                    | `float` | Inclusive upper bound. At least one of min and max is required. The unit depends on the metric preset.                                                                                                                       | `None`     |
| `validate_metric_preset` | `bool`  | Check metric against the list of presets known to pyvespa (see metric_presets()), as the Vespa CLI does. Set to False if Vespa Cloud has added a preset that this pyvespa version does not know about yet. Defaults to True. | `True`     |
...
#### `from_dict(data, validate_metric_preset=True)`
...
### `ApplicationPackage(name, schema=None, query_profile=None, query_profile_type=None, stateless_model_evaluation=False, create_schema_by_default=True, create_query_profile_by_default=True, configurations=None, validations=None, components=None, auth_clients=None, clusters=None, deployment_config=None, services_config=None, query_profile_config=None, include_files=None, production_tests=None)`
...
| Name                              | Type                                              | Description                                                                                                                                                                                                                                                                                                                                                                                                            | Default    |
| --------------------------------- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `name`                            | `str`                                             | Application name. Must start with a lowercase letter, may only contain lowercase letters, digits, underscores and dashes, and may contain no more than 40 characters. Note that this is not the Vespa Cloud application name, which is given to VespaCloud and is subject to stricter rules (no underscores).                                                                                                          | *required* |
| `schema`                          | `list`                                            | List of Schema objects for the application. If None, a default Schema with the same name as the application will be created, with any dashes replaced by underscores, as schema names cannot contain dashes. Defaults to None.                                                                                                                                                                                         | `None`     |
...
| `production_tests`                | `List[ProductionTest]`                            | Production tests to write to tests/production-test/ in the package. They run at each Test step of the deployment_config when deploying to Vespa Cloud prod. If this list is non-empty and deployment_config has no such step (no Test in its steps, or for a VT config no <test> under <prod>), a UserWarning is emitted here at construction time, since the tests would be uploaded but never run. Defaults to None. | `None`     |
...
This is a verbatim copy of `metric-presets.json` from the Vespa CLI, taken at the git ref in `metric_presets_source()`. Vespa Cloud is the authority; if a preset is missing here, pass `validate_metric_preset=False` to `ProductionTest`. See https://docs.vespa.ai/en/reference/applications/testing-production.html#metric-presets.

### `validate_production_test_files(directory, validate_metric_preset=True)`

Validate the production test files in a `tests/production-test/` directory, the way `vespa prod deploy` does before submitting, and return the tests found.

Each `.json`, `.yaml` or `.yml` file may hold one test object or an object with a `tests` list. Legacy step-based tests (with a `steps` field) and files with other extensions are ignored. Nothing is evaluated here; Vespa Cloud runs the tests after deployment.

Parameters:

| Name                     | Type               | Description                                                    | Default    |
| ------------------------ | ------------------ | -------------------------------------------------------------- | ---------- |
| `directory`              | `Union[str, Path]` | The tests/production-test directory.                           | *required* |
| `validate_metric_preset` | `bool`             | Reject metric names not in metric_presets(). Defaults to True. | `True`     |

Returns:

| Type                   | Description                                                 |
| ---------------------- | ----------------------------------------------------------- |
| `List[ProductionTest]` | List\[ProductionTest\]: The validated tests, in file order. |
