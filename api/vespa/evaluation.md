...
### `Vespa(url, port=None, deployment_message=None, cert=None, key=None, vespa_cloud_secret_token=None, output_file=sys.stdout, application_package=None, additional_headers=None, cert_content=None, key_content=None)`
...
| `cert_content`             | `str`  | PEM content of the data plane certificate, and of the key in case 'key_content' is None. Mutually exclusive with 'cert' and 'key'. Useful when the certificate is read from e.g. an environment variable.        | `None`     |
| `key_content`              | `str`  | PEM content of the data plane key. Requires 'cert_content'. Mutually exclusive with 'cert' and 'key'.                                                                                                            | `None`     |
...
#### `syncio(connections=8, compress='auto', session=None, num_retries_429=10)`
...
| `num_retries_429` | `int` | Retries per request on a 429 response or connection error. Defaults to 10; 0 returns every response unchanged. | `10` |
...
#### `query(body=None, groupname=None, streaming=False, profile=False, keep_request_body=False, **kwargs)`
...
| `keep_request_body` | `bool` | If True, store a deep copy of body on the returned response as response.request_body. Defaults to False. Has no effect when streaming=True. | `False` |
...
#### `feed_iterable(iter, schema=None, namespace=None, callback=None, operation_type='feed', max_queue_size=1000, max_workers=8, max_connections=16, compress='auto', num_retries_429=10, **kwargs)`
...
| `compress`        | `Union[str, bool]` | Whether to compress the request body. Defaults to "auto", which will compress if the body is larger than 1024 bytes.                                                                                              | `'auto'`   |
| `num_retries_429` | `int`              | Retries per request on a 429 response or connection error. Defaults to 10; 0 passes every response to the callback unchanged.                                                                                     | `10`       |
...
#### `feed_async_iterable(iter, schema=None, namespace=None, callback=None, operation_type='feed', max_queue_size=1000, max_workers=64, max_connections=1, docv1_retry_policy=None, **kwargs)`
...
| `docv1_retry_policy` | `AsyncRetrying`  | Replaces the default two-layer document/v1 retry (unbounded 429 retry inside 3 attempts on 503/exception). Pass vespa.retries.NO_RETRY to deliver every response, including 429, to the callback unchanged.       | `None`     |
| `**kwargs`           | `dict`           | Additional parameters passed to the respective operation type-specific function (\_data_point).                                                                                                                   | `{}`       |
