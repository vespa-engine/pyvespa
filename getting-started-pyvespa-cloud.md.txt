...
This notebook needs two Python packages: [pyvespa](https://pyvespa.readthedocs.io/) (the API) and `vespacli` (a Python wrapper for the [Vespa CLI](https://docs.vespa.ai/en/vespa-cli.html) that pyvespa calls internally for configuration and credential management).
...
`app` is a [`Vespa`](https://vespa-engine.github.io/pyvespa/api/vespa/application.md#vespa.application.Vespa) instance that lets you query and feed documents. It connects to the secure (mTLS) endpoint where your app is now running:
...
## Example: Querying with a token[¶](#example-querying-with-a-token)
...
vespa_cloud_with_token = VespaCloud( tenant=tenant_name, application=application, key_content=key, application_package=package, auth_client_token_id="default-token", ) vespa_cloud_with_token.deploy()

Now create the token. It's valid for 14 days. Copy the printed value if you want to use it from outside this notebook — Vespa Cloud can't show it to you again.
...
token = vespa_cloud_with_token.create_dataplane_token() print("Token id:", token.id) print("Expires: ", token.expiration)

# token.token holds the secret — capture it now if you need it outside this notebook (Vespa Cloud only shows it once)

token_app = vespa_cloud_with_token.get_application( endpoint_type="token", vespa_cloud_secret_token=token.token, ) with token_app.syncio(connections=1) as session: response = session.query( yql="select * from sources * where userQuery() limit 3", query="How Fruits and Vegetables Can Treat Asthma?", ranking="bm25", ) assert response.is_successful() print(display_hits_as_df(response, ["id", "title"]))
