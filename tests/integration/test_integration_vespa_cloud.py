...
class TestDeployBatchProdWithTests(unittest.TestCase):
    def setUp(self) -> None:
        # Set root to parent directory/testapps/production-deployment-with-tests
        self.application_root = (
            pathlib.Path(__file__).parent.parent
        ...
        for i in range(3):
            ...
            schema_path.write_text(f"# deploy {i}\n" + original_schema)
            build_no = self.vespa_cloud.deploy_to_prod(
                source_url="https://github.com/vespa-engine/pyvespa",
            )
            self.build_nos.append(build_no)
            print("Doing deployment: ", build_no)
    ...
    def test_application_status(self):
        ...
        success = self.vespa_cloud.wait_for_prod_deployment(
            build_no=self.build_nos[0], max_wait=3600 * 4
        )
        if not success:
            self.fail("Deployment failed")
        ...
        if success:
            self.fail("Batched/absorbed deployment should not succeed")

        # Build 3 succeeds
        success = self.vespa_cloud.wait_for_prod_deployment(
            build_no=self.build_nos[2], max_wait=3600 * 4
        )
        if not success:
            self.fail("Deployment failed")
