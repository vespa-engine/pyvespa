...
export const options = {
  scenarios: {
    mtls: { ...scenario, exec: "mtlsScenario" },
    token: { ...scenario, exec: "tokenScenario" },
  },
...
function metrics(kind) {
  return {
    duration: new Trend(`${kind}_req_duration`),
    failed: new Rate(`${kind}_fail_rate`),
    requests: new Counter(`${kind}_reqs`),
    limited: new Counter(`${kind}_rate_limited`),
  };
}
const token = metrics("token");
const mtls = metrics("mtls");

async function feed(url, authHeader, kind, measured) {
  async function stream() {
    while (exec.instance.currentTestRunDuration < measureEndMs) {
      const docId = Math.random().toString(36).slice(2);
      const endpoint = `${url.replace(/\/+$/, "")}/document/v1/msmarco/msmarco/docid/${docId}`;
      const payload = JSON.stringify({
        fields: { id: docId, title: "performance-doc", body: "benchmark run" },
      });
      const res = await http.asyncRequest("POST", endpoint, payload, {
        timeout: "120s",
        headers: {
          "Content-Type": "application/json",
          ...(authHeader ? { Authorization: authHeader } : {}),
        },
        tags: { kind, name: `feed_doc_${kind}` },
      });
...
export async function mtlsScenario() {
  await feed(__ENV.MTLS_URL, null, "mtls", mtls);
}

export async function tokenScenario() {
  await feed(__ENV.TOKEN_URL, __ENV.TOKEN_AUTH_HEADER, "token", token);
}
