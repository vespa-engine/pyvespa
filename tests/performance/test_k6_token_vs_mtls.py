...
from utils.workloads import K6_THRESHOLDS, PROFILE, VALIDITY
...
def _run(endpoints, report_dir, name: str) -> List[LaneResult]:
    token, mtls = run_k6(
        endpoints, PROFILE, report_dir / f"{name}_summary.json", extra_env=None
    )
...
def test_token_vs_mtls_performance_last(
    ...
    last = _run(vespa_cloud_token_endpoints, report_dir, "k6_token_vs_mtls_last")
