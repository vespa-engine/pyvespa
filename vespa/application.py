...
def raise_for_status(
    ...
    Args:
        response (Response): Response object from Vespa API.
        raise_on_not_found (bool): If True, raise HTTPError if status_code is 404.
...
class Vespa(object):
    def __init__(
        ...
        Args:
            ...
            port (int, optional): Vespa endpoint port.
            deployment_message (str, optional): Message returned by Vespa engine after deployment. Used internally by deploy methods.
            cert (str, optional): Path to data plane certificate and key file in case the 'key' parameter is none. If 'key' is not None, this
                should be the path of the certificate file. Typically generated by Vespa-cli with 'vespa auth cert'.
            key (str, optional): Path to the data plane key file. Typically generated by Vespa-cli with 'vespa auth cert'.
            vespa_cloud_secret_token (str, optional): Vespa Cloud data plane secret token.
            output_file (str, optional): Output file to write output messages.
            application_package (str, optional): Application package definition used to deploy the application.

        Returns:
            Vespa: A connection object to the Vespa application.
    ...
    def asyncio(
        ...
        **kwargs,
        ...
        Args:
            ...
            timeout (httpx.Timeout, optional): Timeout object. See https://www.python-httpx.org/advanced/timeouts/. Defaults to 5 seconds.
    ...
    def syncio(
        ...
        Example:
            ...
            See `VespaSync` for more details.

        Args:
            connections (int): Number of allowed concurrent connections.
            total_timeout (int): Total timeout in seconds.
        ...
        Returns:
            VespaSync: Instance of Vespa synchronous layer.
        """
        return VespaSync(
            app=self,
    ...
    def wait_for_application_up(self, max_wait: int = 300) -> None:
        """
        Wait for the application endpoint to be ready (`/ApplicationStatus`).

        Args:
            max_wait (int): Seconds to wait for the application endpoint.

        Raises:
            RuntimeError: If unable to reach the endpoint within `max_wait` seconds or if the client fails to authenticate.
    ...
    def get_application_status(self) -> Optional[Response]:
        ...
        Returns:
            dict: The application status response.
        """
    ...
    def query(
        ...
        Args:
            ...
            groupname (str, optional): The group name used with streaming search.
            kwargs (dict, optional): Extra Vespa Query API parameters.

        Returns:
            dict: The response from the Vespa application.
        """
    ...
    def feed_data_point(
        ...
        **kwargs,
        ...
        """
        Feed a data point to a Vespa app. Creates a new `VespaSync` with connection overhead.
        ...
        Args:
            schema (str): The schema to which the data is sent.
            ...
            namespace (str, optional): The namespace to which the data is sent.
            groupname (str, optional): The group name to which the data is sent.
            compress (Union[str, bool], optional): Whether to compress the request body. Defaults to `"auto"`, which compresses if the body is larger than 1024 bytes.
    ...
    def feed_iterable(
        ...
        **kwargs,
        ...
        """
        Feed data from an iterable of dictionaries to be used in `feed_data_point`.
        ...
        Args:
            iter (Iterable[dict]): An iterable of dictionaries containing the keys `'id'` and `'fields'`  
                to be used in `feed_data_point`. The `'id'` is only the last part of the full document ID,  
                which will be generated automatically by `pyvespa`.
            schema (str): The Vespa schema name to which the data is sent.
            namespace (str, optional): The Vespa document ID namespace. If not provided, the schema is used.
            callback (Callable[[VespaResponse, str], None], optional): A callback function  
                called on each result. The signature should be `callback(response: VespaResponse, id: str)`.
            operation_type (str, optional): The operation to perform. Defaults to `"feed"`.  
                Valid values are `"feed"`, `"update"`, or `"delete"`.
            max_queue_size (int, optional): The maximum size of the blocking queue and max in-flight operations.
            max_workers (int, optional): The maximum number of workers in the thread pool executor.
            max_connections (int, optional): The maximum number of persisted connections to the Vespa endpoint.
            compress (Union[str, bool], optional): Whether to compress the request body.  
                Defaults to `"auto"`, which compresses if the body is larger than 1024 bytes.
            kwargs (dict, optional): Additional parameters passed to the respective operation-specific function.
    ...
    def feed_async_iterable(
        ...
        **kwargs,
        ...
        """
        Feed data asynchronously using `httpx.AsyncClient` with HTTP/2.

        Feeds from an iterable of dictionaries with the keys `'id'` and `'fields'` to be used in `feed_data_point`.  
        The result of each operation is forwarded to the user-provided callback function that can process  
        the returned `VespaResponse`. Prefer using this method over `feed_iterable` when the operation  
        is I/O-bound from the client side.
        ...
        Args:
            ...
            schema (str): The Vespa schema name to which the data is sent.
            namespace (str, optional): The Vespa document ID namespace. If not provided, the schema is used.
            callback (Callable[[VespaResponse, str], Awaitable[None]]): A coroutine function called  
                on each result. The signature should be `callback(response: VespaResponse, id: str)`.
            operation_type (str, optional): The operation to perform. Defaults to `"feed"`.  
                Valid values are `"feed"`, `"update"`, or `"delete"`.
            max_queue_size (int, optional): The maximum number of tasks waiting to be processed.  
                Useful to limit memory usage. Defaults to `1000`.
            max_workers (int, optional): Maximum number of concurrent requests allowed in-flight,  
                controlled by an `asyncio.Semaphore`. Increase this value if the server is scaled  
                to handle more requests.
            max_connections (int, optional): The maximum number of connections passed to `httpx.AsyncClient`.  
                As HTTP/2 is used, only one connection is typically needed.
            kwargs (dict, optional): Additional parameters passed to the respective operation-specific function.
    ...
    async def query_many_async(
        ...
        **query_kwargs,
        ...
        """
        Execute multiple queries asynchronously using `httpx.AsyncClient`.

        The number of concurrent requests is controlled by `max_concurrent`.  
        Each query will be retried up to 3 times using an exponential backoff strategy.

        Args:
            queries (Iterable[dict]): An iterable of query bodies (dictionaries) to be sent.
            num_connections (int, optional): Number of connections to be used in the asynchronous client  
                (uses HTTP/2). Defaults to `1`.
            max_concurrent (int, optional): Maximum number of concurrent requests to be sent.  
                Defaults to `100`. Be cautious when increasing this value.
            client_kwargs (dict, optional): Additional arguments passed to `httpx.AsyncClient`.
            query_kwargs (dict, optional): Additional arguments passed to the `query` method.

        Returns:
            list[VespaQueryResponse]: A list of `VespaQueryResponse` objects.
        """
        results = []
    ...
    def query_many(
        ...
        **query_kwargs,
        ...
        This method wraps `query_many_async` and runs the coroutine using the `asyncio` event loop.  
        The number of concurrent requests is controlled by `max_concurrent`.  
        Each query will be retried up to 3 times using an exponential backoff strategy.
    ...
    def delete_data(
        ...
        **kwargs,
        ...
        """
        Delete a data point from a Vespa application.
        ...
        Args:
            schema (str): The schema from which data is being deleted.
            data_id (str): The unique ID associated with the data point.
            namespace (str, optional): The namespace from which data is being deleted.  
                If not provided, the schema is used.
            groupname (str, optional): The group name from which data is being deleted.
            kwargs (dict, optional): Additional parameters for the HTTP DELETE request.  
                See [Vespa Document API](https://docs.vespa.ai/en/reference/document-v1-api-reference.html#request-parameters).
