...
class TestSimplifiedApplicationPackageAddBertRanking(unittest.TestCase):
    ...
    def test_schema_to_text(self):
        expected_result = (
            ...
            "                        TOKEN_CLS,\n"
            "                    if (d1 < question_length + 1,\n"
            "                        query(bert_tiny_query_token_ids){d0:(d1-1)},\n"
            "                    if (d1 == question_length + 1,\n"
            "                        TOKEN_SEP,\n"
            "                    if (d1 < question_length + doc_length + 2,\n"
            "                        attribute(bert_tiny_doc_token_ids){d0:(d1-question_length-2)},\n"
            "                    if (d1 == question_length + doc_length + 2,\n"
            "                        TOKEN_SEP,\n"
            "                        TOKEN_NONE\n"
            "                    ))))))\n"
            ...
            "        function logit0() {\n"
            "            expression {\n"
            "                sum(tensor(x{}):{x1:onnxModel(bert_tiny).logits{d0:0,d1:0}})\n"
            "            }\n"
            "        }\n"
            "        function logit1() {\n"
            "            expression {\n"
            "                sum(tensor(x{}):{x1:onnxModel(bert_tiny).logits{d0:0,d1:1}})\n"
            "            }\n"
            "        }\n"
            "        first-phase {\n"
