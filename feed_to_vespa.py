...
def vespa_visit(endpoint, namespace, doc_type, continuation=None):
    ...
    return vespa_get(
        endpoint, "document/v1/{0}/{1}/docid".format(namespace, doc_type), options
    )
...
def vespa_remove(endpoint, doc_ids, namespace, doc_type):
    ...
    for doc_id in doc_ids:
        ...
        vespa_delete(
            endpoint,
            "document/v1/{0}/{1}/docid/{2}".format(namespace, doc_type, id),
            options,
        )
...
def get_feed_docids(feed, namespace, doc_type):
    ...
    if doc_type == "doc":
        return set(
            [
                "id:{0}:doc::".format(namespace)
                + find(doc, "fields.namespace")
                + find(doc, "fields.path")
                for doc in feed_json
            ]
        )
    elif doc_type == "term":
        return set(
            [
                "id:{0}:term::".format(namespace) + str(find(doc, "fields.hash"))
                for doc in feed_json
            ]
        )
...
def update_endpoint(endpoint, config):
    ...
    print_header(
        "Retrieving already indexed document ids for endpoint {0}".format(endpoint_url)
    )
    ...
    if do_feed:
        ...
        for index in endpoint_indexes:
            ...
            docids_in_feed = docids_in_feed.union(
                get_feed_docids(index, namespace, doc_type)
            )
        ...
        if len(docids_to_remove) > 0:
            print_header(
                "Removing indexed documents not in feed in {0}".format(endpoint_url)
            )
...
def main():
    ...
    retries = Retry(
        total=10, connect=10, backoff_factor=0.8, status_forcelist=[500, 503, 504, 429]
    )
